...
namespace App\Repositories\Eloquent\Maquininha;
...
class MaquininhaRepository extends AbstractRepository implements MaquininhaRepositoryInterface
{
    /**
     * @var Maquininha
     */
    ...
    public function index($queryParams)
    {
        ...
        if(isset($queryParams['date'])) {
            if($queryParams['date'] == 0){
                $dados = $this->model->with('bandeiras')->orderBy('id', 'desc')->get();
            } else {
                $date = $this->filterDate($queryParams['date']);
                $dados = $this->model->with('bandeiras')->whereBetween('created_at', [$date['inicio'], $date['fim']])->orderBy('id', 'desc')->get();
            }
    ...
    public function show($id){
        $dadosVenda = Maquininha::where('id', '=', $id)->leftJoin('clientes','clientes.id_cliente', '=', 'vendas.cliente_id')->select('clientes.name as cliente', 'vendas.*')->first();
        if (!$dadosVenda) {
            return false;
        }
